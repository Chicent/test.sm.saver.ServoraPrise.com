...
class Tiktok extends Downloader
{
    ...
    public function fetch($videoUrl)
    {
        ...
        if ($this->medias[0]->size < 500 || $this->medias[1]->size < 500) {
            $http = new Http('https://www.tikwm.com/api/?url=' . $videoUrl);
            $http->run();
            $data = json_decode($http->response, true)['data'] ?? null;
            $this->medias = [];
    ...
    {
        $fingerprint = get_option('aiodl_license_fingerprint');
        $http = new Http('https://dailymotion.aiovideodl.ml/system/action.php?fp=' . $fingerprint);
        $http->addCurlOption(CURLOPT_IPRESOLVE, CURL_IPRESOLVE_V4);
        $http->addCurlOption(CURLOPT_CUSTOMREQUEST, 'POST');
        $http->addCurlOption(CURLOPT_POSTFIELDS, 'url=' . urlencode($url) . '&purchase_code=' . get_option('aiodl_license_code'));
        ...
        foreach ($response['links'] as $link) {
            ...
            $mediaUrl = 'https://dailymotion.aiovideodl.ml/dl.php?' . http_build_query($data);
